                        ...
                        <form action="{{ route('blog_categories.update', $category->id ) }}" method="POST">
                                    ...
                                    @foreach( LaravelLocalization::getSupportedLocales() as $key => $lang )
                                        ...
                                        <div class="tab-pane {{ ( $key == LaravelLocalization::getCurrentLocale() ) ? 'active' :'' }}" id="tab_{{ $key }}">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="category_name_{{ $key }}" required class="form-control" value="{{ $item->category_name }}">
                                            </div>
                                            <div class="form-group">
                                                <label for="">{{ trans('product::categories.category_slug') }}</label>
                                                <input type="text" readonly name="category_slug_{{ $key }}" class="form-control" value="{{ $item->category_slug }}">
                                            </div>
                                            <div class="form-group">
                                                <label for="">{{ trans('product::categories.category_description') }}</label>
                                                <textarea id="" name="category_description_{{ $key }}" id="" class="form-control" cols="30" rows="5">{{ $item->category_description }}</textarea>
                                            </div>
                            ...
                            <div class="form-group">
                                <div class="select-single-file">
                                    ...
                                    <div class="img-item">
                                        <img width="100" src="{{ $category->category_image }}" alt="">
                                        <input type="hidden" value="{{ $category->category_image }}" name="image" id="" class="form-control ">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="parent_id" class="form-control" id="">
                                    ...
                                    {!! \App\Helpers\NestableRender::renderSelect($cat, $category->parent_id ) !!}
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="category_status" id="" class="form-control">
                                    <option value="1" {{ ($category->category_status == 1 ) ? 'selected' : '' }}>@lang('app.enabled')</option>
                                    <option value="0" {{ ($category->category_status == 0 ) ? 'selected' : '' }}>@lang('app.disabled')</option>
                                </select>
                            </div>
